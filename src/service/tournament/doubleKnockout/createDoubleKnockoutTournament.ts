...
const getBracketsRoundsAndMatches = (participants: number) => {
    try {
        ...
        throw new Error(
            ",Error in get brackets rounds and matches data "
        );
    }
};

console.log("participants : ", getBracketsRoundsAndMatches(8));
...
const referencingMatchesToNextMatches = async (
    ...
    try {
        ...
        const allRoundsData = await roundModel
            ...
            .populate<{ matches: IMatch[] & mongoose.Document }>("matches") // Ensures matches are populated with full objects
            .session(session);
        allRoundsData.sort(
        ...
        // console.log("round Match Map : ", allRoundsData[0].matches, roundMatchIdsMap, rounds);
        for (let i = 2; i <= rounds.length; i++) {
            ...
            // console.log("matches : round", i, currentRoundMatches);
            let index = 0;
            for (let match = 0; match < currentRoundMatches.length; match++) {
                if (index < prevRoundMatchesIds.length) {
                    ...
                    // console.log('const objects : ', setObj);
                    bulkUpdateMatches.push({
                        updateOne: {
            ...
            // console.log("updated Matches:", currentRoundMatches)
        }
        // console.log("updated Rounds Matches : ", allRoundsData[0].matches, bulkUpdateMatches);
        ...
        // const updatedMatches = await matchModel.find({ roundID: allRoundsData?.map((round) => round?._id) }).session(session);
        // console.log("updated matches after placeholder for winners : ",updatedMatches);
...
const creatingFinalBracketRoundMatch = async (data: finalBracketPayload, session: ClientSession) => {
    try {
        ...
        let finalBracket = finalBracketArr[0] as IRound;
        const str = "K3";
        const matchObj = {
...
const tournamentDoubleKnockoutFormatCreation = async (payload: payloadType) => {
    ...
    try {
        ...
        // Extract the data fields from payload
        console.log("double knockout payload received");
        const data = {
        ...
        // Validate Sport is Exist or not
        ...
        // const totalRounds = Math.ceil(Math.log2(data.participants));
        // const roundNames = getRoundNames(data.participants, totalRounds);
        ...
        // Create Double Knockout Tournament
        ...
        console.log("Double knockout format created");
        // Generate Brackets Round Data For winners, losers
        const roundsData = getBracketsRoundsAndMatches(data.participants);
        // Create Rounds For Winners Bracket
        ...
        // add Referencing of next matches and Add Previous Match Placeholders
        ...
        // const matchesUpdated = await matchModel.find({ roundID: matches[0]?.roundID }).session(session);
        ...
        // Create Rounds For Losers Bracket
        await createRoundAndTheirMatches(losersBracketPayload, session);
        // Add Referencing of Matches into Loser Bracket and Add Previous Match Placeholders
        ...
        // Assign Losers of Winners Bracket into losers Bracket
        ...
        const losersRounds = await assigningLosersIntoLosersBracket(
            winnersRoundMatches,
            losersRoundMatches,
            session
        )
        // Create Final Bracket
        // 17.Creating Final Round and match
        const finalObjData = {
            tournamentID: tourId as string,
        ...
        // 18. Setting Reference of participants Into Final Bracket Match From winners and losers bracket Final match
        ...
        // Add referencing and Placeholder of Previous Bracket Finals
        allRoundsAndMatches[winnersLastRound - 1].matches[0] =
            await allRoundsAndMatches[winnersLastRound - 1].matches[0].save({
                session,
            });
        losersBracketRounds[losersLastRound - 1].matches[0] =
            await losersBracketRounds[losersLastRound - 1].matches[0].save({
                session,
            });
        FinalBracketMatch = await FinalBracketMatch.save({ session });
        // Update Double Knockout and Tournament 
        ...
        // }
        // const roundsIds = allRounds?.map((round) => (round?._id as unknown) as mongoose.Schema.Types.ObjectId);
